...
test-integration: manifests generate setup-envtest ## Run envtest integration tests.
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name minato-builder
	$(CONTAINER_TOOL) buildx use minato-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm minato-builder
	rm Dockerfile.cross

.PHONY: build-installer
build-installer: manifests generate ## Generate a consolidated YAML with CRDs and deployment.
	mkdir -p dist
	helm template minato chart --namespace minato > dist/install.yaml
...
deploy: manifests ## Deploy controller to the K8s cluster specified in ~/.kube/config using Helm.
	helm upgrade --install minato chart --namespace $(NAMESPACE) --create-namespace

.PHONY: undeploy
undeploy: ## Undeploy controller from the K8s cluster specified in ~/.kube/config using Helm.
	helm uninstall minato --namespace $(NAMESPACE) || true
...
.PHONY: buf-generate
buf-generate: ## Generate Go code from protobufs.
	@command -v protoc-gen-go >/dev/null 2>&1 || { \
		echo "Missing protoc-gen-go; install with 'go install google.golang.org/protobuf/cmd/protoc-gen-go@v1.36.8'"; \
		exit 1; \
	}
	@command -v protoc-gen-go-grpc >/dev/null 2>&1 || { \
		echo "Missing protoc-gen-go-grpc; install with 'go install google.golang.org/grpc/cmd/protoc-gen-go-grpc@v1.5.1'"; \
		exit 1; \
	}
	$(BUF) generate
